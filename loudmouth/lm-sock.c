...
#ifndef G_OS_WIN32        /* on windows turn off the gettext() and do not link with libintl */
...
_lm_sock_library_init (void)
{
	...
	if (LOBYTE (data.wVersion) != 2 ||
		...
		lm_verbose ("Socket library version is not sufficient!\n");//! rxt
		WSACleanup ();
		return FALSE;
	}
#endif /* G_OS_WIN32 */
		       ...
		       gboolean block)
{
	...
	//res = ioctlsocket (sock, FIONBIO, &mode);//! rxt!
